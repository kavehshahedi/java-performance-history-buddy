...
def get_function_name(function):
    ...
    function_name = ''.join(function.find('name').itertext()).strip()
    function_return_type = ''.join(function.find('type').itertext()).strip()
...
if __name__ == '__main__':
    for project in projects:
        ...
        for commit in commits:
            ...
            if not commit.stats.files or not any(file.endswith('.java') for file in commit.stats.files):
            ...
            changed_files = [file for file in commit.stats.files if file.endswith('.java')]
            ...
            for file in changed_files:
                ...
                try:
                    if previous_commit:
                        diff = repo.git.diff(f'{previous_commit.hexsha}..{commit.hexsha}', file, unified=0)
                    else:
                    ...
                    write_error(commit_folder, 'git diff', commit.hexsha, previous_commit.hexsha, [f'File: {file}'])
                ...
                process = subprocess.run(['srcml', 'temp.java', '--position'], capture_output=True) 
                xml = process.stdout.decode('utf-8')
                xml = re.sub('xmlns="[^"]+"', '', xml, count=1)

                root = ET.fromstring(xml)
                if root is None:
                    write_error(commit_folder, 'srcml', commit.hexsha, previous_commit.hexsha, [f'File: {file}'])
                    print(f'Error in commit {commit.hexsha} for file {file}')
                    continue
                    ...
                    for line_number in line_numbers:
                        if int(pos_start) < line_number < int(pos_end):
                            ...
                            try:
                                ...
                                write_error(commit_folder, 'method_changes', commit.hexsha, previous_commit.hexsha, [f'File: {file}', f'Function: {method_name}'])
            ...
            if method_changes:
                ...
                with open(os.path.join(commit_folder, 'commit_details.txt'), 'w') as f:
                    ...
                    f.write(f'Previous Commit: {previous_commit.hexsha if previous_commit else None}\n')
