...
class ProjectChangeMiner:
    ...
    def mine(
        self,
        force: bool = False,
        custom_commits: Optional[list[str]] = None,
        ...
        for commit_index, commit in enumerate(
            repo.iter_commits(self.project_branch), start=1
        ):
            ...
            changed_files = [
                ...
                if not any(substring in str(file).lower() for substring in ("/test",))
            ]

            # Now we get the refactoring changes
            refactoring_miner = RefactoringMinerService(
                project_path=self.project_path)
            all_refactorings = refactoring_miner.mine(
            ...
            #     for file_name, methods in cr.items():
            #         srcml = SrcMLService()
            #         if commit_ == commit.hexsha:
            #             # We are in the new file
            #             readed_file = (
            #                 str(repo.git.show(f"{commit.hexsha}:{file_name}"))
            #                 .encode("utf-8", errors="ignore")
            #                 .decode("utf-8")
            #             )
            #         else:
            #             readed_file = (
            #                 str(repo.git.show(f"{previous_commit.hexsha}:{file_name}"))
            #                 .encode("utf-8", errors="ignore")
            #                 .decode("utf-8")
            #             )

            #         file_methods = srcml.get_methods(readed_file)
            ...
            #             print(full_name, comparable_name)

            if restructured_refactorings:
                # Save the method changes in a json file
                FileUtils.write_json_file(
                    os.path.join(commit_folder, "method_changes.json"), restructured_refactorings
                )
                ...
                commit_info = {
                    ...
                    "previous_commit": (
                        previous_commit.hexsha if previous_commit else None
                    ),
                    "author": f"{commit.author.name} <{commit.author.email}>",
                    "date": commit.authored_datetime.strftime("%Y-%m-%d %H:%M:%S"),
                    "message": commit.message,
                    "metadata": refactoring_metadata

                }
                FileUtils.write_json_file(
                    os.path.join(commit_folder, "commit_details.json"), commit_info
                )
            ...
            #     try:
            #         new_file = (
            #             str(repo.git.show(f"{commit.hexsha}:{file}"))
            #             .encode("utf-8", errors="ignore")
            #             .decode("utf-8")
            #         )
            ...
            #             old_file_name = file
            #         old_file = (
            #             str(repo.git.show(f"{previous_commit.hexsha}:{old_file_name}"))
            #             .encode("utf-8", errors="ignore")
            #             .decode("utf-8")
            #         )
            #     except:
            #         self.__write_error(
            #             commit_folder, "git show", commit.hexsha, "None", []
            #         )
            #         Printer.error(
            #             f"Error in commit {commit.hexsha}",
            #             num_indentations=self.printer_indent,
            #         )
            #         continue
            ...
            #         delete=True, mode="w", suffix=".java"
            #     )
            #     old_file_path = tempfile.NamedTemporaryFile(
            #         delete=True, mode="w", suffix=".java"
            #     )
            ...
            #     different_methods = java_service.get_different_methods(
            #         new_file_path.name, old_file_path.name
            #     )
            #     if different_methods is None:
            #         self.__write_error(
            #             commit_folder,
            #             "get_different_methods",
            #             commit.hexsha,
            ...
            #     different_methods = [
            #         diff for diff in different_methods if diff["second"]
            #     ]
            ...
            #     if self.use_llm:
            #         new_file_methods = srcml_service.get_methods(
            #             new_file, with_body=True
            #         )
            #         old_file_methods = srcml_service.get_methods(
            #             old_file, with_body=True
            #         )
            ...
            #                 method_name = java_service.convert_method_signature(
            #                     method_.split("{")[0]
            #                 )
            ...
            #             is_significant = self.llm_service.is_code_change_significant(
            #                 new_method, old_method, wrap_codes=False
            #             )
            ...
            #     for file_, commit_, diff_key in [
            #         (file, commit, "first"),
            #         (old_file_name, previous_commit, "second"),
            #     ]:
            #         file_methods = srcml_service.get_methods(
            #             repo.git.show(f"{commit_.hexsha}:{file_}"),
            #             remove_comments=False,
            #         )

            #         for method_ in file_methods:
            #             converted_method_name = java_service.convert_method_signature(
            #                 method_
            #             )
            ...
            #                     method_changes[file][previous_commit.hexsha].add(
            #                         method_
            #                     )
            ...
            #     if data[commit.hexsha] or data[previous_commit.hexsha]
            # }

            # # Convert the sets to lists
            # method_changes = {
            #     file: {
            #         commit.hexsha: list(data[commit.hexsha]),
            #         previous_commit.hexsha: list(data[previous_commit.hexsha]),
            #     }
            #     for file, data in method_changes.items()
            # }
            ...
            #         new_method_changes[commit_hash] = new_method_changes.get(
            #             commit_hash, {}
            #         )
            #         new_method_changes[commit_hash][file] = methods
            # method_changes = new_method_changes
            
            # delete_empty_commit_folder(commit_folder)

        return num_successful_commits


def delete_empty_commit_folder( commit_folder):
    """
    Delete the commit folder if it's empty.

    :param commit: A commit folder
    ...
    if os.path.exists(commit_folder) and not os.listdir(commit_folder):
        try:
            shutil.rmtree(commit_folder)
            print(f"Deleted empty folder: {commit_folder}")
            return True
        except OSError as e:
            print(f"Error deleting folder {commit_folder}: {e}")
            return False
    else:
        if not os.path.exists(commit_folder):
            print(f"Folder does not exist: {commit_folder}")
        else:
            print(f"Folder is not empty: {commit_folder}")
        return False
