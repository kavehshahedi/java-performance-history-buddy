...
class JavaService:
    ...
    def get_different_methods(self, first_code_path: str, second_code_path: str) -> Union[list[dict], None]:
        ...
        first_output = subprocess.check_output(['java', '-jar', self.jpb_path, '-get-methods-hash', first_code_path]).decode('utf-8').strip()
        second_output = subprocess.check_output(['java', '-jar', self.jpb_path, '-get-methods-hash', second_code_path]).decode('utf-8').strip()
